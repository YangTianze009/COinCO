...
class FakeDetectionEvaluator:
    def __init__(self, methods, base_pred_path, gt_path, context_data_path, out_of_context_path, all_masks_path, enhancement_factor=1.3):
        ...
        Args:
            ...
            out_of_context_path: Path to out_of_context_results.csv
    ...
    def load_prediction(self, img_path):
        ...
        # Change extension from .png to .npy
    ...
    def get_out_of_context_objects(self, coco_index):
        """
        Get list of out-of-context objects for a given coco_index
        """
        row = self.out_of_context_data[self.out_of_context_data['coco_index'] == coco_index]
        if row.empty:
            return []
        
        objects_str = row['out_of_context_objects'].iloc[0]
        if pd.isna(objects_str):
            return []
        
        # Split objects and clean up any whitespace
        objects = [obj.strip() for obj in objects_str.split(',')]
        return objects

    def get_combined_mask(self, coco_index, object_names):
        """
        Get combined mask for multiple objects
        """
        if not object_names:
            return None
            
        masks = []
        mask_dir = os.path.join(self.all_masks_path, str(coco_index))
        
        for obj_name in object_names:
            obj_name = obj_name.strip()
            mask_path = os.path.join(mask_dir, f"{obj_name}.png")
            ...
            if os.path.exists(mask_path):
                try:
                    ...
                    print(f"Error loading mask for {obj_name} in {coco_index}: {str(e)}")
                    continue
    ...
    def enhance_prediction(self, pred, img_name):
        """
        Enhance prediction based on context and out-of-context objects
        """
        try:
    ...
    def save_results(self, results):
        ...
        save_dir = f"evaluation_results/our/{self.enhancement_factor}"
        os.makedirs(save_dir, exist_ok=True)
